<!-- ici Team_view-->
<div class="container mw-100" id="container_team">
	<div>
		<a id="retour" class="invisible" href="#top"></a>
	</div>
	<div class="row align-items-center">
		<div class="col-12">
			<div class="mx-auto">
				<h4 id="team1">TEAM SENIOR</h4>
				<img class="img-fluid" src="<?php echo base_url()?>assets/images/team/pirates_team_Senior.jpg" alt="Pirates_baseball_team_Senior">
				<p>Franck Bonneville, Saville Hervieux, Germain Dufour, Charles Aubé, Chappell Guibord, Emmanuel Duval, Lucien Rémy, Georges Dujardin, Moïse Bonnot, Djeferson Cellier, Stanislas Quint, Isaac Bousquet, Maxime Dimont</p>
			</div>
		</div>
	</div>
	<div class="row align-items-center">
		<div class="col-12">
			<div class="mx-auto">
				<h4>TEAM SENIOR DH</h4>
				<img class="img-fluid" src="<?php echo base_url()?>assets/images/team/pirates_team_SeniorDH.jpg" alt="Pirates_baseball_team_SeniorDH">
				<p>Jean-Pierre Tomas, Joël Stuart, Damien Celice, Josué Alard, Frédéric Lagarde, Lambert Ouvrard, Stanislas Devereux, Georges Dujardin, Moïse Bonnot, Mathias Bessette, Stanislas Quint, Roland Bescond, Charles Mossé, Isaac Bousquet, Roland Bescond</p>
			</div>	
		</div>
	</div>	
	<div class="row align-items-center">
		<div class="col-12">
			<div class="mx-auto">
				<h4>TEAM 15 U</h4>
				<img class="img-fluid" src="<?php echo base_url()?>assets/images/team/pirates_team_15U.jpg" alt="Pirates_baseball_team_15U">
				<p>Clara Duval, Berthe Ballouhey, Anna Fouquet, Brigitte Bourseiller, Solenn Lefrançois, Roger Ancel, Robert Boisselot, Léon Lagarde, François Gaubert, Abelin Jacquet</p>
			</div>	
		</div>
	</div>
	<div class="row align-items-center">
		<div class="col-12">
			<div class="mx-auto">
				<h4>TEAM 12 U</h4>
				<img class="img-fluid" src="<?php echo base_url()?>assets/images/team/pirates_team_12U.jpg" alt="Pirates_baseball_team_12U">
				<p>Franck Bonneville, Saville Hervieux, Germain Dufour, Charles Aubé, Chappell Guibord, Emmanuel Duval, Thibault Laframboise, Lucien Rémy, Georges Dujardin, Moïse Bonnot, Djeferson Cellier, Stanislas Quint</p>
			</div>	
		</div>
	</div>
	<div class="row align-items-center">
		<div class="col-12">
			<div class="mx-auto">
				<h4>TEAM 9 U</h4>
				<img class="img-fluid" src="<?php echo base_url()?>assets/images/team/pirates_team_9U.jpg" alt="Pirates_baseball_team_9U">
				<p>Thibault Laframboise, Lucien Rémy, Georges Dujardi, Moïse Bonnot, Djeferson Cellier, Stanislas Quint, Isaac Bousquet, Maxime Dimont</p>
			</div>	
		</div>
	</div>
	<div class="row align-items-center">
		<div class="col-12">
			<div class="mx-auto">
				<h4 id="team6">TEAM SOFTBALL FEMININ</h4>
				<img class="img-fluid" src="<?php echo base_url()?>assets/images/team/pirates_team_SoftballFem.jpg" alt="Pirates_baseball_team_SoftballFem">
				<p>Célia Daucourt, Thérèse Clérisseau, Elise Loupert, Abélia Badeaux, Bérengère Delafose, Alexia Bombelles, Émilienne Neri, Laëtitia Boudreaux</p>
			</div>	
		</div>
	</div>	
	<div class="row align-items-center">
		<div class="col-12">
			<div class="mx-auto">
				<h4>TEAM SOFTBALL MIXTE</h4>
				<img class="img-fluid" src="<?php echo base_url()?>assets/images/team/pirates_team_SoftballMixte.jpg" alt="Pirates_baseball_team_SoftballMixte">
				<p>Bérengère Delafose, Alexia Bombelles, Émilienne Neri, Germain Dufour, Charles Aubé, Emmanuel Duval, Thibault Laframboise, Lucien Rémy, Georges Dujardin, Solenn Lefrançois, Roger Ancel, Robert Boisselot, Léon Lagarde, Moïse Bonnot, Djeferson Cellier, Stanislas Quint, Isaac Bousquet</p>
			</div>	
		</div>
	</div>
</div>